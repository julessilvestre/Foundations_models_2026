...
class MaskGIT(nn.Module):
    ...
    def __init__(
        ...
        self.positional_embedding = nn.Parameter(torch.randn(seq_len, dim)) #Define the learnable positional embedding
        self.mask_token = nn.Parameter(torch.randn(dim)) # Define the learnable mask token
        self.trunk = TransformerTrunk(dim, depth, head_dim, mlp_ratio, use_bias) #Define the transformer trunk

        self.out_norm = LayerNorm(dim) #Define the output layer normalization. Use the LayerNorm class defined in modeling/transformer_layers.py
    ...
    def generate_random_mask(self, seq: torch.Tensor) -> torch.BoolTensor:
        ...
        # Mask the first num_masked positions in each row according to the random ranking
        mask = ranks < num_masked.unsqueeze(1)
    ...
    def compute_ce_loss(self, logits: torch.Tensor, target_seq: torch.LongTensor, ignore_index: int = -100) -> torch.Tensor:
        ...
        logits = logits.transpose(1, 2) #to reshape to (B, V, L)
    ...
    def get_maskgit_schedule(self, mask: torch.BoolTensor, num_steps: int = 8) -> List[int]:
        ...
        schedule = torch.full((num_steps,), base)
    ...
    def generate(
        ...
        for step, k in enumerate(schedule):
            ...
            masked_indices = torch.where(mask.squeeze(0))[0]
            ...
            confidence = torch.max(masked_logits, dim=1)[0]
            ...
            topk = torch.topk(confidence, k)[1]
            
            selected_positions = masked_indices[topk,]
